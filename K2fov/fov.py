...
class KeplerFov:
    ...
    def isOnSiliconList(self, ra_deg, dec_deg, padding_pix=DEFAULT_PADDING):
        ...
        for channel in set(ch):
            ...
            if channel > 84:
                continue
            out[mask] = self.colRowIsOnSciencePixelList(
                col[mask], row[mask], padding_pix
            )
        return out

    def getChannelColRowList(
        self, ra, dec, wantZeroOffset=False, allowIllegalReturnValues=True
    ):
        ...
        for channel in set(ch):
            mask = ch == channel
            col[mask], row[mask] = self.getColRowWithinChannelList(
                ra[mask], dec[mask], channel, wantZeroOffset, allowIllegalReturnValues
            )
        return (ch, col, row)
    ...
    def getColRowWithinChannelList(
        self, ra, dec, ch, wantZeroOffset=False, allowIllegalReturnValues=True
    ):
    ...
    def getChannelColRow(
        ...
        col, row = self.getColRowWithinChannel(
            ra, dec, ch, wantZeroOffset, allowIllegalReturnValues
        )
        return (ch, col, row)
    ...
    def colRowIsOnSciencePixel(self, col, row, padding=DEFAULT_PADDING):
        ...
        """
        if col < 12.0 - padding or col > 1111 + padding:
            return False
    ...
    def colRowIsOnFgsPixel(self, col, row, padding=-50):
        ...
        boolean
        """
        if col < 12.0 - padding or col > 547 + padding:
            return False
    ...
    def getRaDecForChannelColRow(self, ch, col, row, oneOffsetPixels=True):
        ...
        if ch > 84:
            return self.getRaDecForFgsChannelColRow(ch, col, row, oneOffsetPixels)
        ...
        colFrac = (col - 17.0) / (1106.0 - 17.0)
        rowFrac = (row - 25.0) / (1038.0 - 25.0)
    ...
    def getRaDecForFgsChannelColRow(self, ch, col, row, oneOffsetPixels=True):
        ...
        colFrac = (col - 0.0) / (547.0)
        rowFrac = (row - 0.0) / (527.0)
    ...
    def getChannelAsPolygon(self, chNumber, maptype=None):
        ...
        idx = np.where(radec[:, 2].astype(int) == chNumber)[0]
    ...
    def plotPointing(self, maptype=None, colour="b", mod3="r", showOuts=True, **kwargs):
        ...
        radec = self.currentRaDec
        for ch in radec[:, 2][::4]:
            idx = np.where(radec[:, 2].astype(int) == ch)[0]
    ...
    def plotSpacecraftYAxis(self, maptype=None):
        ...
        a, d = gcircle.sphericalAngDestination(
            self.ra0_deg, self.dec0_deg, -yAngle_deg, 12.0
        )
    ...
    def getWcsForChannel1(self, ch):
        ...
        CD *= self.plateScale_arcsecPerPix / 3600.0
